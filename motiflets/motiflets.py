...
def compute_distances_with_knns_full(
    ...
    bin_size = int(np.ceil(ts.size / n_jobs))
...
@njit(fastmath=True, cache=True)
def apply_exclusion_zone(D_stitched, idx_stitched, knns_stitched, m, ts_stitched):
...
@njit(fastmath=True, cache=True)
def extract_stitched_time_series(
        ts, m,
        D_knn,
        knns,
        kth_extent):
    ...
    return idx_stitched, ts_stitched
...
def _argknn(dist, k, m, lowest_dist=np.inf, slack=0.5):
    ...
    new_k = min(len(dists) - 1, 2 * k)
    dist_pos = np.argpartition(dists, new_k)[:new_k]
    dist_sort = dists[dist_pos]
    ...
    for i in range(len(dist_sort)):
        ...
        d = dists[pos]
        if (not np.isnan(d)) and (not np.isinf(d)) and (d <= lowest_dist):
            idx[count] = pos
            count = count + 1
...
def search_k_motiflets_elbow(
    ...
    if backend == "pyattimo":
    ...
    elif backend in ["default", "scalable", "stitch"]:
        idx_stitched = None
        data_ = data_raw
        ...
        for test_k in np.arange(k_max_ - 1, 1, -1):
            preprocessing = distance_preprocessing(data_, m)
...
def stitch_and_refine(
    ...
    # TODO: change to scalable implementation???
    D_stitched, knns_stitched = compute_distances_with_knns_full(
        ts_stitched,
        m=m,
        k=k_max,
